...
\Title{Evaluation of DigitSeis Software for Building a Vectorised Data Corpus of Historical Seismograms}

% MDPI internal command: Title for citation in the left column
\TitleCitation{Evaluation of DigitSeis Software for Building a Vectorised Data Corpus of Historical Seismograms}
  ...
  \vspace{3mm}
     \begin{subfigure}[h]{0.7\textwidth}
         \centering
        		 ...
        		 \caption{Overlapped traces which cause a problem during vectorising with selected enlarged fragment of seismogram. Here: UCC19540112Gal\_E\_0750.TIFF}
     \end{subfigure}
...
Therefore, we evaluated the 1-second time mark dimensions which show the breaks of the line along its course. The recording contained time marks at every minute. The whole line takes 30 min intervals on the seismogram which shows the occurrence of the earthquake signals for each line (Fig. \ref{fig:07}). On the original TIFFs of the seismograms, the time is marked according to the Greenwich time and recorded at 30 mm/min. Marking time was performed by measuring width and offset of the gaps in the traces and recording the values in the menu bar. Since the majority of lines had time gaps, the time mark width was set to a negative number, Fig. \ref{fig:08}. 
...
Validating the results and checking quality control of the digitised traces have been performed using Python \cite{van1995python}. For export and graphical visualisation of the MAT files we sued library Matplotlib \cite{Hunter}. For tuning the parameters of the digitising process in the next series of recoding we performed the post-processing of the result using graphical visualisation and the analysis of the performed vectorisation by exporting the file of MAT format into Python, Fig. \ref{fig:20} and \ref{fig:21}. 

The digitised time gaps are visible in Fig. \ref{fig:20} as green dots, while blue dots signify the selected dots, and red dots signify the remaining noise and edge regions where the automatic classification was more difficult compared to the central parts of seismograms. The misclassified pixels were induced by the distortions of paper which caused the erroneous recognition of pixels in the edge of paper (red dots). Such pixels are classified as noise in the segments of trace in a way that it erroneously depicts the edges of seismic trace, as shown in Fig. \ref{fig:20}. 

The average length of segments divided automatically by DigitSeis was assessed and visualised using Matplotlib, Fig. \ref{fig:21}. Because the program classifies seismic traces and short time gaps based on their geometrical characteristics and threshold, the quality of scanned paper may affect the automatic recognition and interpretation. Therefore, some segments were merged into one continuous line, while others were broken apart. So, Fig. \ref{fig:21} illustrate the analysis of the performance of DigitSeis in vectorising segments of a trace: the average length of the segment identified by the program as 'trace' and the frequency of the segments per hour.  
...
\begin{figure}[H]
     ...
     \begin{subfigure}[H]{0.7\textwidth}
      	\centering
         	\includegraphics[width=13.0cm]{fig_21a.png}
         	\caption{Frequency of segment length visualized in Matplotlib: size of various segments in a trace}
     \end{subfigure}
     \vspace{1mm}   

     \begin{subfigure}[H]{0.7\textwidth}
         \centering
        		 \includegraphics[width=13.0cm]{fig_21b.png}
        		 \caption{Analysis of statistics on segment length per hours of recording in Matplotlib}
     \end{subfigure}
     \caption{Validating the results of segments tracing in Python for selected seismogram} \label{fig:21}
\end{figure}

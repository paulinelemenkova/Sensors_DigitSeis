...
\begin{figure}[H]
	\includegraphics[width=10.0cm]{fig_01a.jpg}
        \caption{Study %MDPI: Please add explanatory information about the start, squares in circle in the middle of every subfigure. <-- Figure in subplot 2 is deleted.
 area: Location of Uccle seismic station, ROB on the topographic map of Belgium. Software: {GMT version 6.1.1, creator: P. Wessel et al. \cite{Wessel}, University of Hawai'i at Mānoa, location -- Honolulu, Hawai'i, U.S.A.} %MDPI: Please state the software version number, creator, and the location (city, country) from where the software was sourced. <-- Authors: information added: GMT version 6.1.1, creator: P. Wessel et al. from the University of Hawai'i at Mānoa(2019), location -- Honolulu, Hawai'i, U.S.A. Citation added in the bibliography (the last item).
...
\setcounter{subfigure}{3}
     ...
     \end{subfigure}
     \vspace{6pt}
     \caption{Examples of the raw data: old analog seismograms recorded by Uccle seismic station, Belgium, 1954, scanned and saved as TIFF files. Some images have visible distortions and defects. (\textbf{a}) %MDPI: We moved the information for figure to caption. <-- Authors: agree.
...
The dataset included a collection of 145 images covering the period from 1 January 1954 to 12 March 1954. Some of the images are well preserved{, while others} have distortions and defects visible on the aged paper, Figure \ref{fig:04}. The scanned images of old seismograms have different features and characteristics, which increase technical difficulties of processing such data: line breaks, distortions, defects, blurred lines, spots, etc. For instance, some data had defects due to the record imperfectness or{ various cases related to }storage{. }Oftentimes{, }the traces are slanted as shown, e.g., in Figure %MDPI: Figures 6, 10 and 11 are mentioned after figure 4, please revise the order. <-- Authors: citation to Figure 10 is deleted from this part of text. It is cited later close to the figure itself on p. 13 and 14. The same for Figure 11: it is cited later on p. 13.
 \ref{fig:06}, while{ }others appear more horizontal{, see }Figure %MDPI: Figure citation should be in order, Figure 7 should be mentioned after Figure 6. Please revise the citations and make sure the figures are mentioned in numerical order. <-- Authors: now corrected. Figure 7 is cited after Figure 6.
...
We used the DigitSeis software version 1.5, developed by Bogiatzis, P. and Ishii, M. from the Department of Earth and Planetary Sciences, Harvard University, Cambridge, U.S.A. %MDPI: Please state the software version number, creator, and the location (city, country) from where the software was sourced. <-- Authors: information added as follows: version -- v1.5, creator -- Bogiatzis, P. and Ishii, M. from the Department of Earth and Planetary Sciences, Harvard University, location -- Cambridge, U.S.A. 
The DigitSeis is developed as {an }instrument for digitising {scanned }seismograms{. }It{ }extracts {the data }from the{ }seismograms {and processes them }as an image matrix. {The }DigitSeis converts scanned raster images of analog seismograms from {the }original TIFF format into the convenient digital format using {algorithms of }signal processing. The approach of {the }DigitSeis is based on the principles of semi-automated algorithms of signal processing as time series. It applies minor human supervision, mostly for {the }adjustment of {the }functionality in selected process {steps} and {for }monitoring technical workflow \linebreak (Figure \ref{fig:06}). 
...
\setcounter{subfigure}{1}
     ...
     \end{subfigure}
     \vspace{6pt}
     \caption{Seismogram %MDPI: 1. Please use commas to separate thousands for numbers with five or more digits (not for four digits) in the picture. e.g., "10000" should be "10,000". 2. The contents of this figure 6b  are not legible. In order to convert a clear PDF document, whilst retaining its high quality, we kindly request the provision of figures and schemes at a sufficiently high resolution (min. 1000 pixels width/height, or a resolution of 300 dpi or higher). 
     % <-- Authors: Figure 6b is replaced with higher resolution (300 dpi) and replotted with commas to separate thousands  for numbers with five or more digits.
...
\begin{figure}[H]
  ...
  \vspace{3mm}
     \begin{subfigure}[h]{0.7\textwidth}
        		 ...
        		 \caption{} 
     \end{subfigure}
     \caption{Analysing seismogram in paper-based format: traces and time gaps. (\textbf{a}) %MDPI: Moved into caption. <-- Authors: agree.
 Original scanned seismogram (UCC19540116Gal\_E\_0820.tif). (\textbf{b}) Enlarged %MDPI: if the different color lines and circle need explanation, please confirm <-- Authors: they don't need explanation since different colours and circles are used to highlight the minuscule breaks of seismic traces for convenience of reader. 
...
\begin{figure}[H]
     ...
     \end{subfigure}
     \vspace{3pt}
     \caption{Classification %MDPI: We moved the figure after its first citation. <-- Authors: agree.
     of seismogram processed by DigitSeis. (\textbf{a}) %MDPI: The contents  of this figure are not legible. In order to convert a clear PDF document, whilst retaining its high quality, we kindly request the provision of figures and schemes at a sufficiently high resolution (min. 1000 pixels width/height, or a resolution of 300 dpi or higher). <-- Authors: Figures 8a and 8b are replaced with original files of higher quality (resolution 300 dpi).
...
\begin{figure}[H]
  ...
  \caption{Threshold %MDPI: Please change hyphen into minus if it's possible. <-- Authors: Figure 9 is corrected and replaced.
...
\setcounter{subfigure}{1}
     ...
     \end{subfigure}
     \vspace{3pt}
     \caption{Results %MDPI: 1. For numbers with more than 4 digits, comma is required, please change 10000 into 10,000.}  <-- Authors: Figures 10a and 10b are replotted as requested, with used commas to separate thousands for numbers with five or more digits. %MDPI: Please use commas to separate thousands for numbers with five or more digits (not for four digits) in the picture. e.g., "10000" should be "10,000".  <-- Authors: answered above. 2. the red font not clear, please replace a shaper  <-- Authors: Figures 10a and 10b are replaced with higher resolution.
...
\begin{figure}[H]
     ...
     \end{subfigure}
     \vspace{3pt}
     \caption{Results %MDPI: 1. The bottom of subfigure a is incomplete and subfigure b is unclear. please replace a shaper picture. <-- Authors: Figures 11a and 11b are replaced with higher resolution. Figure 11a is replotted with corrected X-axis.
   %MDPI: 2. Please use commas to separate thousands for numbers with five or more digits (not for four digits) in the picture. e.g., "10000" should be "10,000" <-- Authors: Figure 11b is replotted with used commas to separate thousands for numbers with five or more digits
      %MDPI: 3. if yellow color line need explanation, please confirm <-- Authors: explanation added: Yellow color signifies a selected segment on the trace line.
 of the scanned seismogram classification processed by DigitSeis. (\textbf{a}) Results of the classified image with shown yellow segments of the identified trace (enlarged fragment). Here: example of file UCC19540109Gal\_E\_0812.tif. Yellow color signifies a selected segment on the trace line.(\textbf{b}) Classified seismogram with traces saved in binary format 0--1. Here: example of file UCC19540109Gal\_E\_0812.tif (seismogram recorded on 9 January 1954).} \label{fig:11}
\end{figure}
...
\begin{figure}[H]
     ...
     \end{subfigure} 
     \vspace{3pt}
     \caption{Results %MDPI: 1.  many numbers in The left and right  of subfigure ``a'' is incomplete .  please replace a shaper picture. <-- Authors: Figure 12a is replaced with file with sharp left and right annotations and 300 dpi sharp resolution. 
   %MDPI: 2. Please use commas to separate thousands for numbers with five or more digits (not for four digits) in the picture. e.g., "10000" should be "10,000" <-- Authors: Figure 12a is replotted with used commas to separate thousands for numbers with five or more digits, as recommended.
 of the paper-based seismogram vectorisation processed by DigitSeis. (\textbf{a}) Digitised traces upon completion of the automated vectorisation. Some time gaps in the upper left part of the image were too small and not clearly visible for {the }automatic discrimination between the trace and {the }dark background. In these cases, gaps required manual correction to identify time intervals. \linebreak (\textbf{b}) {The }enlarged view of the automatically recognised digitised traces displayed by {the }lines of various colours, zero-lines for each trace (cyan, dashed lines, numbered from top to bottom) and time gaps (vertical yellow dashes).} \label{fig:12}
\end{figure}
...
\setcounter{subfigure}{1}
     ...
     \end{subfigure}
     \vspace{3mm}   
     \caption{Identified %MDPI: 1. Please provide more clear version of figure. <-- Authors: Figures 13a and 13b replaced with 600 dpi resolution (files 2,3 and 3,4 Mb for each subplot). 2. if the different colored shape need explanation, please confirm, <-- Authors: Explanation is added: Differently colored lines are used to distinguish the segments of the traces. 
 traces for selective correction and re-digitising using Correct Trace mode. \linebreak (\textbf{a}) Identified wrong vector direction of line crossing individual traces. (\textbf{b}) Detected misclassifications caused erroneous digitising. The gaps on the zero-lines (small yellow boxes) show the gaps that existed in the old paper in the original image itself. Differently colored lines are used to distinguish the segments of the traces. } \label{fig:13}
\end{figure}
...
\setcounter{subfigure}{1}
     ...
     \begin{subfigure}[h]{0.7\textwidth}
        		 ...
        		 \caption{}
     \end{subfigure} 
     \caption{Correcting %MDPI: 1. Please provide a more clear version of subfigure a. <-- Authors: Figure 14a is replaced with a file with higher resolution. 2. if the different colored shape need explanation, please confirm, <-- Authors: Explanation added: "The differently colored lines are used to distinguish the segment of trace lines."
 misclassified traces with wrong direction based on colour and geometric pixel's characteristics. The pixels on the monochrome image were assigning to categories using threshold values to 1 (white) and all other pixels to 0 (black) for automated detection of lines. The differently colored lines are used to distinguish the segment of trace lines. {(\textbf{a})} Overlap of line traces unrecognised during digitising: one segment of trace went steeply downwards and merged with another trace. {(\textbf{b})} Enlarged view of the manually corrected entangled traces.} \label{fig:14}
\end{figure}
...
\setcounter{subfigure}{1}
     \begin{subfigure}[h]{0.7\textwidth}
        		 ...
        		 \caption{}
     \end{subfigure}
     \caption{Correcting %MDPI: The top of subfigure a seem incomplete, please revise. <-- Authors: Figure 15a is replotted.
 trace for the selected segments. (\textbf{a}) Merging the trace initially broken into the three separate parts (three small yellow boxes). (\textbf{b}) Reclassification of the selected segment and digitising the centroid of the trace line.} \label{fig:15}
\end{figure}
...
\setcounter{subfigure}{1}
     ...
     \begin{subfigure}[h]{0.7\textwidth}
        		 ...
        		 \caption{}
     \end{subfigure}
     \caption{Corrected digitised traces upon completion of the {semi-}automated vectorisation. \linebreak (\textbf{a}) Corrected digitised traces upon {the }completion of the {semi-}automated vectorisation {process}. (\textbf{b}) Visible SDT lines (bold) after {the semi-}automated vectorisation.} \label{fig:16}
\end{figure}
...
\setcounter{subfigure}{0}
     ...
     \begin{subfigure}[h]{0.7\textwidth}
        		 ...
        		 \caption{}
     \end{subfigure}
     \caption{Classified %MDPI: 1. many numbers in The left and right  of subfigure ``b'' is incomplete .  please replace a shaper picture. <-- Authors: Figure 17b is replaced with file of with sharper resolution and corrected annotations.
   %MDPI: 2. Please use commas to separate thousands for numbers with five or more digits (not for four digits) in the picture. e.g., "10000" should be "10,000" <-- Authors: Figures 17a and 17b are corrected with used commas to separate thousands for numbers with five or more digits.
 image (\textbf{a}) and digitised results (\textbf{b}). Here: UCC19540311Gal\_E\_0727 \linebreak (11 March 1954). (\textbf{a}) Classified image recognised by DigitSeis into 'traces' and 'noise' (Here: {the image }UCC19540311Gal\_E\_0727.TIFF). (\textbf{b}) {The }distinct coloured traces and minute time gaps are visible on {a }digitised image (Here: {the resulted file in a MATLAB format, }UCC19540311Gal\_E\_0727.mat).} \label{fig:17}
\end{figure}
...
\setcounter{subfigure}{1}
     ...
     \end{subfigure}
     \vspace{3mm}
     \caption{Seismogram %MDPI: 1.  many numbers in The left and right  of subfigure ``a,b,c'' is incomplete .  please replace a shaper picture. <-- Authors: Figures 18a, 18b and 18c are provided as original files with 300 dpi resolution and corrected left and right annotations.
   %MDPI: 2. Please use commas to separate thousands for numbers with five or more digits (not for four digits) in the picture. e.g., "10000" should be "10,000" <-- Authors: Figures 18a, 18b and 18c are corrected with used commas to separate thousands for numbers with five or more digits.
...
\begin{figure}[H]
     \begin{subfigure}[H]{0.7\textwidth}
         	\includegraphics[width=11.0cm]{fig_20a.jpg}
         	\caption{}
     \end{subfigure}
     \vspace{1mm}   

     \begin{subfigure}[H]{0.7\textwidth}
        		 \includegraphics[width=11.0cm]{fig_20b.jpg}
        		 \caption{}
     \end{subfigure}
     \caption{Controlling %MDPI: 1. Please use commas to separate thousands for numbers with five or more digits (not for four digits) in the picture. e.g., "10000" should be "10,000" <-- Authors: Figures 20a and 20b are replotted with used commas to separate thousands for numbers with five or more digits. 
     %MDPI:2. please add explanation for different color dot <-- Authors: Added explanation for different color dots: Different color dots are used to separate minute marks (green), hour marks (blue) and margin values and outliers (red).
 digitising results using Python (Matplotlib library). {(\textbf{a})} Quality control for time gaps: missed marks in unrecognised segments. {(\textbf{b})} Correctly identified time gaps controlled by Python's Matplotlib. Different color dots are used to separate minute marks (green), hour marks (blue) and margin values and outliers (red).} \label{fig:20}
\end{figure}
...
\newblock %\hl{cp1--cp2.} %MDPI: Is this information necessary? <-- Authors: not necessary, deleted.
...
Somville, O. %MDPI: please confirm if Refs. 98 and 99 are  same.  if yes,  please replace or delete the duplicated reference. If one of them is deleted, please update the reference list as well as reference order in the whole text. Please make sure the first citation of all references are mentioned in numerical order. <-- Authors: No, they are not the same. These are two different references: 1) https://adsabs.harvard.edu/full/1922AnOB....1..285S 2) https://adsabs.harvard.edu/full/1922AnOB....1..391S

...
\abstract{
Archived seismograms recorded in 20th century present \textcolor{blue}{a} valuable source of information for monitoring earthquake activity. However, old data\textcolor{blue}{, which} are only available as scanned paper-based images \textcolor{blue}{should} be digitised and converted from raster to vector format \textcolor{blue}{prior to reuse for geophysical modelling}. Seismograms have special characteristics \textcolor{blue}{and specific features }recorded by \textcolor{blue}{a} seismometer and encrypted in the images: \textcolor{blue}{signal trace lines}, minute time gaps, timing and wave amplitudes. This information should be recognised and interpreted \textcolor{blue}{automatically when processing archives of seismograms containing large collections of data. The} objective was to automatically digitise historical seismograms \textcolor{blue}{obtained from the archives of the Royal Observatory of Belgium (ROB). The images were originally }recorded by \textcolor{blue}{the} Galitzine seismometer in 1954 in Uccle seismic station, Belgium. A dataset included 145 TIFF images \textcolor{blue}{which required automatic approach of data processing}. Software for digitising seismograms are limited and many have disadvantages. We \textcolor{blue}{applied the} DigitSeis \textcolor{blue}{for machine-based vectorisation and reported here} a full workflow\textcolor{blue}{of data processing. This} included pattern recognition, classification, digitising, corrections and converting TIFFs to \textcolor{blue}{the} digital \textcolor{blue}{vector format}. The generated contours of signals were presented as time series and converted into digital format (mat files) which indicated information on ground motion signals contained in analog seismograms. \textcolor{blue}{We performed the quality control of the digitised traces in Python to evaluate the discriminating functionality of seismic signals by DigitSeis.} We shown a robust approach of DigitSeis as a powerful toolset for processing analog seismic signals. The graphical visualisation of signal traces and analysis of the performed vectorisation results shown that the algorithms of data processing performed accurately and can be recommended in similar applications of seismic signal processing in future related works \textcolor{blue}{in geophysical research}.}  
...
The study was performed in the Université Libre de Bruxelles, École polytechnique de Bruxelles, Laboratory of Image Synthesis and Analysis (LISA), Belgium. We used archives obtained as a courtesy of \ac{ROB}, Department of Seismology \& Gravimetry, \ac{UCC}, Figure \ref{fig:01}. \hl{The data processing }is \hl{performed }within the framework of the SeismoStorm project.
...
\hl{Seismic }traces and short time gaps \hl{were classified }based on their geometrical characteristics and threshold\hl{. Therefore, }the quality of scanned paper may affect the automatic recognition and interpretation. \hl{For instance}, some segments were merged into one continuous line, while others were broken apart. Figure \ref{fig:21} \hl{illustrates }the analysis of the performance of DigitSeis in vectorising \hl{the }segments of a trace for the two examples (UCC19540109Gal\_E\_0812.mat and UCC19540311Gal\_E\_0727\_280.mat): the average length of the \hl{identified segments of seismic lines }as 'trace' and the frequency of the segments per hour\hl{, respectively}.  

Seismic lines were separated\hl{ into the }segments\hl{, }according to the traced lines (a 30-minute trace on each seismograms) \hl{using algorithms of }DigitSeis. The program searched for the correct path of the line and zero-lines using \hl{the defined }parameters\hl{ of the skeleton of line. }Separating \hl{the } traces and noise from the time gaps and \hl{the }background on the paper was performed automatically. Therefore, screening and examining each pixel of the raster image was performed through the defined classifier thresholds\hl{ which }resulted in variations of \hl{selected }segments of the traces. 

\hl{The }parameters for image classification included image threshold \hl{at }82\%\hl{ and the }time mark width for gaps as -12 (in some cases up to -22, depending on the quality of scanned paper). The setup parameters included threshold and intervals of \hl{the }time gaps corresponding to the minute marks made by the drum of the seismometer. Some gaps were identified better\hl{, }while others\hl{ }were blurred, Figure \ref{fig:20}. As a results, \hl{the }discrimination of the traces \hl{varied }based on \hl{the }admissible properties using \hl{image }thresholds, as assessed by Python, Figure \ref{fig:21}. 

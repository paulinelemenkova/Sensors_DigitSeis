...
After all the traces were digitised, the time was calculated using 'Calculate Timing' function, Figure \ref{fig:18}. The time calculation properties were setup using a menu where the program asks for the default time and time increment (in the demonstrated case, January 9, 1954), Figure Figure \ref{fig:18} (a). Thus, we set up the absolute time for one record as a reference point which was then highlighted in red. For the setup of time marks we identified the time gaps that correspond to the beginning of the minute and provided the absolute time using a special menu, Figure \ref{fig:18} (b). It included the time in HH:MM format and year-month-day period that exist in each seismogram. The time and date associated with the time mark were entered and the time marks were recalculated, Figure \ref{fig:18} (c).
...
\funding{This research received funding from the Belgian Science Policy Office (BELSPO) for SeismoStorm project. We thank R. S. M. De Plaen and T. Lecocq for the dataset.}
